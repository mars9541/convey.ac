                                        ...
                                        <div class="card-body color-black-light">
                                            ...
                                            <div class="text-center m-t-20" id="universal_code">
                                                @if(!isset($universal_code) || isset($universal_code) && $universal_code->message == '')
                                                    <form action="{{route('advisors.generate_universal_code')}}" method="post">
                                                        @csrf
...
@section('script')
    ...
    <script>
        ...
        $('#unique_code_table').DataTable({
            ...
            autoWidth: false,
            ajax:{
                url: "{{ route('advisors.get_unique_code') }}",
                method:'post',
                data:{
        ...
        $('#unique_code_generate').on('click', function(){
            ...
            {
                $.ajax({
                    url:"{{ route('advisors.save_unique_code') }}",
                    method:"POST",
                    data: {
                        email:$('#email').val(),
        ...
        {
            $.ajax({
                url:"{{ route('advisors.get_received_amount')}}",
                method:"POST",
                data: {
    ...
    <script>
        ...
        $('#received_table').DataTable({
            ...
            autoWidth: false,
            ajax:{
                url: "{{ route('advisors.receive_his') }}",
                method:'post',
                data:{
                    _token: $('meta[name="_token"]').attr('content')
                }
    ...
    <script>
        ...
        function withdraw_request(){
            ...
            if (/^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/.test($('#receiver_account').val()))
            {
                if(parseInt(withdraw_amount) >= 10000){
                    ...
                    $('#withdraw_btn').prop('disabled', true);
                    $.ajax({
                        url:"{{ route('advisors.withdraw_request')}}",
                        method:"POST",
                        data: {
        ...
        $('#withdraw_table').DataTable({
            ...
            autoWidth: false,
            ajax:{
                url: "{{ route('advisors.withdraw_his') }}",
                method:'post',
                data:{
                    _token: $('meta[name="_token"]').attr('content')
                }
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        if(row.status == '0') {
                            ...
                            return `<a href="{{url('advisors/advice_note_download')}}/`+data+`" class="text-convey-green">payment advice note</a>`;
                        }
                    }
                }
            ]
