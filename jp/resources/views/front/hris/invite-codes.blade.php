                                        ...
                                        <div class="card-body color-black-light">
                                            ...
                                            <div class="text-center m-t-20" id="universal_code">
                                                @if(!isset($universal_code) || isset($universal_code) && $universal_code->message == '')
                                                    <form action="{{route('hris.generate_universal_code')}}" method="post">
                                                        @csrf
                            ...
                            <div class="table-rep-plugin">
                                <div class="table-responsive mb-0" data-pattern="priority-columns">
                                    <div class="col-md-12 text-center m-t-40">
                                        ...
                                        <div class="text-center m-t-20">
                                            <p>(minimum amount to be withdrawn ¥10000).</p>
                                        </div>
...
@section('script')
    ...
    <script>
        ...
        $('#unique_code_table').DataTable({
            ...
            autoWidth: false,
            ajax:{
                url: "{{ route('hris.get_unique_code') }}",
                method:'post',
                data:{
        ...
        $('#unique_code_generate').on('click', function(){
            ...
            if(($('#email').val()!='') && ($('#unique_code').val()!=''))
            {
                ...
                $.ajax({
                    url:"{{ route('hris.save_unique_code') }}",
                    method:"POST",
                    data: {
                        email:$('#email').val(),
        ...
        {
            $.ajax({
                url:"{{ route('hris.get_received_amount')}}",
                method:"POST",
                data: {
    ...
    <script>
        ...
        function withdraw_request(){
            ...
            {
                if(parseInt(withdraw_amount) >= 10000){
                    $('.loading-show2').css('display','contents');
                    $('#withdraw_btn').prop('disabled', true);
                    $.ajax({
                        url:"{{ route('hris.withdraw_request')}}",
                        method:"POST",
                        data: {
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        if(row.status == '0') {
                            ...
                            return `<a href="{{url('hris/advice_note_download')}}/`+data+`" class="text-convey-green">payment advice note</a>`;
                        }
                    }
                }
            ]
