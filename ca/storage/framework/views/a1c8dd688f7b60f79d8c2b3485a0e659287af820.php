                                ...
                                <div class="card-body color-black-light">
                                    ...
                                    <div class="text-center m-t-20" id="universal_code">
                                        <?php if(!isset($universal_code) || isset($universal_code) && $universal_code->message == ''): ?>
                                            <form action="<?php echo e(route('business.generate_universal_code')); ?>" method="post">
                                                <?php echo csrf_field(); ?>
...
<script>
    ...
    $('#unique_code_table').DataTable({
        ...
        autoWidth: false,
        ajax:{
            url: "<?php echo e(route('business.get_unique_code')); ?>",
            method:'post',
            data:{
    ...
    $('#unique_code_generate').on('click', function() {
        ...
        {
            $.ajax({
                url:"<?php echo e(route('business.save_unique_code')); ?>",
                method:"POST",
                data: {
                    email:$('#email').val(),
    ...
    {
        $.ajax({
            url:"<?php echo e(route('business.get_received_amount')); ?>",
            method:"POST",
            data: {
...
<script>
    ...
    $('#received_table').DataTable({
        ...
        autoWidth: false,
        ajax:{
            url: "<?php echo e(route('business.receive_his')); ?>",
            method:'post',
            data:{
                    _token: $('meta[name="_token"]').attr('content')
                }
...
<script>
    ...
    function withdraw_request(){
        ...
        if (/^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/.test($('#receiver_account').val()))
        {
            if(parseInt(withdraw_amount) >= 200){
                ...
                $('#withdraw_btn').prop('disabled', true);
                $.ajax({
                    url:"<?php echo e(route('business.withdraw_request')); ?>",
                    method:"POST",
                    data: {
    ...
    $('#save_bank').on('click', function(){
        ...
        $.ajax({
            url:"<?php echo e(route('business.add_paypal')); ?>",
            method:"POST",
    ...
    $('#withdraw_table').DataTable({
        ...
        autoWidth: false,
        ajax:{
            url: "<?php echo e(route('business.withdraw_his')); ?>",
            method:'post',
            data:{
                    _token: $('meta[name="_token"]').attr('content')
                }
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        if(row.status == '0') {
                            ...
                            return `<a href="<?php echo e(url('business/advice_note_download')); ?>/`+data+`" class="text-convey-green">payment advice note</a>`;
                        }
                    }
                }
                ]
...
<?php echo $__env->make('layouts.master-business', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/sites/15a/f/fc05c3f35d/public_html/ca/resources/views/front/business/invite-codes.blade.php ENDPATH**/ ?>