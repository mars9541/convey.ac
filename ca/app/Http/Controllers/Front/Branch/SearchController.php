...
class SearchController extends Controller
{
    ...
    {
        if($request->NI_number != 'ABC123456') {
        ...
        $messages = array(
            'NI_number.min' => 'The Social Insurance Number for the Canada is set so it must be at least '.$min_num.' characters long.',
        );
...
//                date_modify($today, "-10 minutes");
    ...
    public function request_record_unlock(Request $request)
    {
        ...
        $branch = Branch::find(session('user')->CBR_id);

        $cbr_data = CBR_table::find(session('user')->id);
        ...
        if(!is_null($lock_citizen_info)) {
            ...
            $citizen->requested_business_id = session('user')->CBR_id;
            $citizen->update();
