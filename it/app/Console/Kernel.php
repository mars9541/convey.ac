...
//            $full_name = $user_data->firstname.' '.$user_data->lastname;
    ...
    public function citizen_signup_email()
    {
        ...
        $data1 = User::where('user_type', 'citizen')
            ->where('reminder_sent_on', null)
            ->where('created_at', '<=', date('Y-m-d H:i:s', strtotime($now . "-1 days")))
            ->where('created_at', '>', date('Y-m-d H:i:s', strtotime($now . "-2 days")))
            ->get();
...
*/
    ...
    public function AutoRemoveTestRecord()
    {
        ...
        {
            RDB_perm::where('NI_identity_number', CommonController::EncryptNI($value->NI_number))
                ->where('time_stamp', '<', date('Y-m-d H:i:s', strtotime($now . "-1 day")))
                ->delete();
            RecordHistory::where('NI_identity_number', CommonController::EncryptNI($value->NI_number))
                ->where('time_stamp', '<', date('Y-m-d H:i:s', strtotime($now . "-1 day")))
                ->delete();
        }
    ...
    public function AutoRemoveEmailInOctoPus()
    {
        $error_str = '';
        $now = date('Y-m-d H:i:s');
        $list_id_array = array('23f26dea-fd00-11ec-9258-0241b9615763',
            '09f2b27d-fd00-11ec-9258-0241b9615763',
            'feb4b206-fcff-11ec-9258-0241b9615763',
            'a060b09a-8671-11ec-9835-06b4694bee2a',
            'ee21f442-fcff-11ec-9258-0241b9615763');

        $new_user_list = User::where('signup_date', '>=', date('Y-m-d H:i:s', strtotime($now . "-15 minutes")))->get();

        foreach($new_user_list as $key => $value) {
            foreach($list_id_array as $list_id) {
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, 'https://emailoctopus.com/api/1.6/lists/'.$list_id.'/contacts?api_key='.env('MAIL_OCTOPUS_API_KEY'));
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                $result = curl_exec($ch);

                if (curl_errno($ch)) {
                    $error_str = 'Error:' . curl_error($ch);
                }

                curl_close($ch);
                $email_ocotpus_list_array = json_decode($result);

                if(isset($email_ocotpus_list_array->data)) {
                    foreach($email_ocotpus_list_array->data as $list_info) {

                        if($list_info->email_address == $value->email) {
                            $ch = curl_init();

                            curl_setopt($ch, CURLOPT_URL, 'https://emailoctopus.com/api/1.6/lists/'.$list_id.'/contacts/'.$list_info->id.'?api_key='.env('MAIL_OCTOPUS_API_KEY'));
                            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
