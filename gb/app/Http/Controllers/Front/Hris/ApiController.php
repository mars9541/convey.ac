...
class ApiController extends Controller
{
    ...
    {
        $data = Guide::where('flag','api_connect_page_of_hris')->first()->content;
        return view('front.hris.api-connect',compact('data'));
    }
    ...
    public function get_api_record_list(Request $request)
    {
        if(request()->ajax())
        {
            $data1 = RDB_perm::where('HRIS_id',Auth::user()->id)
                ->where('connection_type', 'API');
            $data2 = RDB::where('HRIS_id',Auth::user()->id)
                ->where('connection_type', 'API')
                ->union($data1)
    ...
    public function get_api_record_error_list(Request $request)
    {
        if(request()->ajax())
        {
            $data = RDB::where('HRIS_id',Auth::user()->id)
                ->where('connection_type', 'API')
                ->orderByDesc('time_stamp')
                ->get();
            $tabledata['data'] = $data;
