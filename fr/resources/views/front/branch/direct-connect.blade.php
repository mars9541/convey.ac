@extends('layouts.master-business-branch')
@section('css')
<style>
    .ion-edit:hover{
        cursor: pointer;
                            ...
                            <div class="card m-b-20">
                                <div class="card-body">
                                    ...
                                    <div class="col-md-12 display-inline">
                                        <label class="col-form-label offset-sm-2 p-3 color-black-light display-inline">Date of birth and NIR can not be edited.</label>
                                    </div>
...
<script>
    ...
    function first_load(){
        ...
        $.ajax({
            url: "{{ route('branch.get_employees') }}",
            method: "POST",
            dataType: "json",
            success:function(html)
            {
                ...
                $.ajax({
                    url: "{{ route('branch.get_record_types') }}",
                    method: "POST",
                    dataType: "json",
                    success:function(html)
                    {
                        if(html.data.length > 0)
                        {
                            ...
                            var draft_info = '{{$draft->RECORD_content}}';

                            if(draft_info != '') {
                                m_draft_flag = 1;

                                var html_content = '{{$draft->RECORD_content}}';
                                html_content = html_content.replace(/&lt;/g, "<");
                                html_content = html_content.replace(/&gt;/g, ">");
                                html_content = html_content.replace(/&quot;/g, "\"");
                                html_content = html_content.replace(/&amp;nbsp;/g, " ");
                                $('#text_answer').summernote('code', html_content);
                                $('#employees').val('{{$draft->NI_identity_number}}').change();
                                $('#employees').attr('disabled', 'disabled');
                                $('#record_types').val('{{$draft->connection_type}}').change();
                                ...
                                if(record_type == 0) {
                                    ...
                                    $.ajax({
                                        url: "{{ route('branch.get_qa_type_info') }}",
                                        method: "POST",
                                        data:
                                            {
...
<script>
    ...
    $('#employees_table').DataTable({
        ...
        autoWidth: false,
        ajax:{
            url: "{{ route('branch.employee_list') }}",
            method: 'post',
            data: {
                    _token: $('meta[name="_token"]').attr('content')
            }
    ...
    $('#employee_form').on('submit', function(event) {
        ...
        {
            $.ajax({
                url: "{{ route('branch.employee_update') }}",
                method: "POST",
                data: new FormData(this),
                contentType: false,
                cache: false,
    ...
    function edit_employee(id) {
        $('#e_form_result').html('');
        $.ajax({
            url:"{{ url('branch/get_employee') }}/" + id,
            dataType:"json",
            success:function(html) {
    ...
    function delete_employee(id) {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "{{ url('branch/delete_employee') }}/" + id,
                    dataType: "json",
                    success:function(data) {
                        var html = '';
                        if(data.success)
...
<script>
    ...
    $('#record_history_table').DataTable({
        ...
        searching: false,
        ajax: {
            url: "{{ route('branch.record_history_list') }}",
            method: 'post',
            data: function ( d ) {
                        ...
                        if(row.max_version == 'yes') {
                            if(row.full_name == "Employee Off-boarded") {
                                return '<a href="javascript:view_record(\''+data+'\')" class="text-dark"><i class="ion-eye"></i></a> | ' +
                                    '<a href="{{url("branch/recordHistoryDownload")}}/'+data+'" title="Print" class="text-dark"><i class="ion-printer"></i></a>';
                            } else {
                                return '<a href="javascript:view_record(\''+data+'\')" class="text-dark"><i class="ion-eye"></i></a> | ' +
    ...
    {
        $.ajax({
            url: "{{ url('branch/get_record_history_content') }}/" + id,
            dataType: "json",
            success:function(html) {
                ...
                render: function (data, type, row) {
                    ...
                    // else
                        return '<a href="javascript:view_record_type(\''+data+'\')" class="text-dark edit" ><i class="ion-eye"></i></a> | ' +
                            '<a href="{{url("branch/recordTemplateDownload")}}/'+data+'" class="text-dark"><i class="ion-printer"></i></a>';
                }
            }
        ]
    });
