...
class SearchController extends Controller
{
    ...
    {
        if($request->National_Number != 'ABC123456789ABC') {
        ...
        $messages = array(
            'National_Number.min' => 'The numéro d\'inscription au répertoire for the France is set so it must be at least '.$min_num.' characters long.',
        );
            ...
            foreach ($parent_data as $item) {
                ...
                if(count($max_version) > 0) {
                    foreach ($max_version as $key => $child) {
                        ...
                        if($request->National_Number != 'ABC123456789ABC789ABC') {
                            ...
                            $child_record_pattern_array->Record_creators_CBR_id = "FR000000";
...
//                    array_push($item_child, $child_record_pattern_array);
                ...
                if($request->National_Number != 'ABC123456789ABC789ABC') {
                    ...
                    $record_pattern_array->Record_creators_CBR_id = "FR000000";
...
//                date_modify($today, "-10 minutes");
    ...
    public function request_record_unlock(Request $request)
    {
        ...
        if($request->National_Number == 'ABC123456789ABC') {
            return response()->json(['errors' => ['This test number is internally controlled and cannot be added to ABC123456789ABC']]);
        }
        ...
        if(!is_null($lock_citizen_info)) {
            ...
            if($citizen->record_lock == '0') {
                ...
                if($flag) {
                    $citizen->requested_business_id = null;
                    $citizen->update();
