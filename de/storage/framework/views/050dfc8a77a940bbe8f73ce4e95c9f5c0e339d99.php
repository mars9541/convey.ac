                                        ...
                                        <div class="card-body color-black-light">
                                            ...
                                            <div class="text-left m-t-20">
                                                <p class="color-black-light">
                                                    Our universal codes expire after 10 days, you can generate one code every 30 days and one code can be shared with multiple businesses.
                                                </p>
                                            </div>
                                            <div class="text-center m-t-20" id="universal_code">
                                                <?php if(!isset($universal_code) || isset($universal_code) && $universal_code->message == ''): ?>
                                                    <form action="<?php echo e(route('hris.generate_universal_code')); ?>" method="post">
                                                        <?php echo csrf_field(); ?>
                                                        <button type="submit" class="btn bg-emerald text-white btn-wd-200 waves-effect waves-light">Click to reveal Your Code</button>
                                                    </form>
                                                <?php else: ?>
                                                    <?php if($universal_code->view_code == 'yes'): ?>
                                                        <div class="row">
                                                            <input type="text" class="form-control display-inline col-sm-4 offset-4" value="<?php echo e($universal_code->invite_code); ?>" readonly>
                                                            <span class="display-inline col-sm-4 text-left m-t-5 color-black-light" ><?php echo e($universal_code->message); ?></span>
                                                        </div>
                                                    <?php else: ?>
                                                        <span class="text-center color-black-light"><?php echo e($universal_code->message); ?></span>
                                                    <?php endif; ?>
                                                <?php endif; ?>
                                            </div>
                                        </div>
                                ...
                                <div class="col-md-6">
                                    <div class="card text-center">
                                        <div class="card-body color-black-light">
                                            ...
                                            <div class="text-left m-t-20">
                                                <p class="color-black-light">
                                                    Unique codes last for 10 days and can be sent to just one other business, the advantage with unique codes is that after you send your email containing the link, we will send 2 reminder emails to your contacts.
                                                </p>
                                            </div>
                                            ...
                                            <div class="text-center m-t-20">
                                                <button type="button" class="btn bg-emerald text-white btn-wd-200 waves-effect waves-light" id="add_unique_code">
                                                    Add another line
                                                </button>
                                            </div>
                                        </div>
                                        ...
                                        <div id="bank_account_box" class="card m-b-20 text-center"  >
                                            ...
                                            <div id="bank_account_details">
                                                <div class="color-black-light row">
                                                    ...
                                                    <label class="col-form-label col-md-3 text-left color-black-light" id="paypal_account"><?php echo e($paypal_account); ?></label>
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn bg-convey-green text-white" id="unique_code_generate">
                        <div style="display:none;" class="loading-show2">
                            <span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
                        </div>
                        Save</button>
                ...
                <div class="modal-body">
                    <form id = "bank_form" action="post" onsubmit="return false;">
                        <div class="color-black-light row">
                            ...
                            <input type="text" class="col-sm-8 form-control" name="receiver_account" id="receiver_account"  value="<?php echo e($paypal_account); ?>" />
...
<?php $__env->startSection('script'); ?>
    ...
    <script>
        ...
        $('#unique_code_table').DataTable({
            ...
            autoWidth: false,
            ajax:{
                url: "<?php echo e(route('hris.get_unique_code')); ?>",
                method:'post',
                data:{
        ...
        $('#unique_code_generate').on('click', function(){
            ...
            if(($('#email').val()!='') && ($('#unique_code').val()!=''))
            {
                ...
                $.ajax({
                    url:"<?php echo e(route('hris.save_unique_code')); ?>",
                    method:"POST",
                    data: {
                        email:$('#email').val(),
        ...
        {
            $.ajax({
                url:"<?php echo e(route('hris.get_received_amount')); ?>",
                method:"POST",
                data: {
    ...
    <script>
        ...
        $('#received_table').DataTable({
            ...
            autoWidth: false,
            ajax:{
                url: "<?php echo e(route('hris.receive_his')); ?>",
                method:'post',
                data:{
                    _token: $('meta[name="_token"]').attr('content')
                }
    ...
    <script>
        ...
        function withdraw_request(){
            ...
            if (/^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/.test($('#receiver_account').val()))
            {
                if(parseInt(withdraw_amount) >= 100){
                    ...
                    $('#withdraw_btn').prop('disabled', true);
                    $.ajax({
                        url:"<?php echo e(route('hris.withdraw_request')); ?>",
                        method:"POST",
                        data: {
        ...
        $('#save_bank').on('click', function(){
            ...
            $.ajax({
                url:"<?php echo e(route('hris.add_paypal')); ?>",
                method:"POST",
        ...
        $('#withdraw_table').DataTable({
            ...
            autoWidth: false,
            ajax:{
                url: "<?php echo e(route('hris.withdraw_his')); ?>",
                method:'post',
                data:{
                    _token: $('meta[name="_token"]').attr('content')
                }
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        if(row.status == '0') {
                            ...
                            return `<a href="<?php echo e(url('hris/advice_note_download')); ?>/`+data+`" class="text-convey-green">payment advice note</a>`;
                        }
                    }
                }
            ]
...
<?php echo $__env->make('layouts.master-hris', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/virtual/vps-91c4dd/d/d2263f56e5/public_html/de/resources/views/front/hris/invite-codes.blade.php ENDPATH**/ ?>